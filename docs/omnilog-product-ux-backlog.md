...
- **Implementation note:** The games-only boolean becomes a four-section preference (`Seccions a l'inici`) in Settings → `Preferències`, filtering **all four Home carousels** rather than only `Ara mateix`. Home carries no control — only a notice when a filter is actually on.
  ...
  - **Home discloses the filter, and only then.** A persistent invisible filter makes the dashboard quietly lie — hide `Jocs`, forget, start a game, and it silently never reaches `Ara mateix`. `DashboardFilterNotice` renders `L'inici no mostra: Jocs · Mostra-ho tot` above the carousels **only when something is hidden**, so the default state costs nothing and the filtered state is never a mystery. `Mostra-ho tot` clears the whole set in one tap. The old boolean had this same trap, capped at one carousel; the filter now reaches four, so the disclosure earns its keep.
  ...
  - **Objectives and analytics are deliberately out of scope.** The preference means "which shelves does Home draw from", not "recompute my year". Filtering `El teu any en contingut` would make `33 completats aquest any` a different claim than the identically-labelled figure on Stats, and objectives already carry their own scope.
  - **Hiding every section is allowed.** Both daily carousels then explain that the filter hid their contents, `En pausa`/`Completats recentment` drop out as they already do when empty, and the notice offers the way back.
  - **The Settings switch is replaced, not merely moved.** This removes a real desync defect: both old copies seeded from `SharedPreferences` once (`rememberSaveable` on Home, `remember` in Settings) and neither observed the file, so toggling in one did not update an already-composed other. Now that the writer (Settings) and the reader (Home) are on different screens, correctness *requires* observation: `rememberHiddenDashboardSections` registers an `OnSharedPreferenceChangeListener` in a `DisposableEffect`, so Home reflects a change made on the other screen whether or not it stayed in composition.
  - **Shared code:** the preference name, keys, migration, read/write, the observing helper, and `DashboardSectionChips` all live in `ui/common/DashboardSectionPreferences.kt`, since Settings and Home now both touch this state.
  - **Persistence and migration:** `dashboard_hidden_sections` stores *hidden* sections, so a medium added to the library later appears by default. `hide_games_from_active` migrates on first read (`true` → `{Games}`) and the old key is removed.
...
- **Verification note — partial; device QA pending.** `:app:compileDebugKotlin` and `:app:testDebugUnitTest` pass on the final code. The **chip-row revision** was fully verified on a connected device against the real library, and the parts it shares with the final design are therefore exercised: the device held `hide_games_from_active=true`, and after install the preference file contained exactly `dashboard_hidden_sections={Games}` with the old key gone; hiding `Anime` dropped it from both `Ara mateix` and `Següent a la llista` while the objectives card kept its full totals; hiding everything left both filtered-empty messages with objectives and analytics intact; the state survived `am force-stop` and relaunch. The device preference was restored to its original `{Games}` afterwards. **Not yet exercised on hardware** (the device was disconnected before the rework could be re-run): the Settings `Seccions a l'inici` row, the `DashboardFilterNotice` and its `Mostra-ho tot` action, and — most importantly — the Settings→Home propagation through the new preference listener, which is the one genuinely new mechanism and the most likely thing to be wrong.
- **Copy:** `home_active_hide_games` removed. `home_hidden_sections_notice` (`L'inici no mostra: %1$s`) is phrased to avoid Catalan gender/number agreement across section names of differing gender. `home_active_filtered_empty` reworded from `No hi ha elements actius visibles.` to name the cause, and `home_planned_filtered_empty` added to match — the planned carousel previously showed its `Res planificat` copy regardless of filtering.
