...
/**
 * The item's genres, directly under the cover.
 *
 ...
 *
 * The fill is a solid blend rather than a translucent accent. Part of this row lands on the foot of
 * the artwork, and a wash there picks up whatever is behind each pill — the same thing that made
 * the collection pill unreadable before it went solid.
 *
 * Scrolls sideways and bleeds off the screen edge, so a long genre list stays one line and shows
 * that it continues rather than reflowing into a block that competes with the title.
 */
@Composable
fun DetailGenreRow(
    genres: List<String>,
